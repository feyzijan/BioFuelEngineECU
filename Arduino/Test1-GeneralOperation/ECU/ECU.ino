...
void read_sensors() {
  /*
  IAT = pulseIn(iat_pin, HIGH,32)/2040.0*5.0*8.38 + 273.15 + 48.935 ;
  //T= 8.38 Vıat + 48.935 + 273.15
  MAP = pulseIn(map_pin, HIGH,32)/2040.0*5.0/2.91*100000;
  //P = V/2.91 * 10^5     -----2.91: 10e5Pa, 0:0
  ECT = pulseIn(ect_pin, HIGH,32)/2040.0*5.0*26.155*-1 + 145.29 + 273.15;
  // T = -26.155V + 145.29 + 273.15
  Lambda = pulseIn(lambda_pin, HIGH,32)/2040.0*5.0*15.04/5.0 + 7.35;
  // AFR = V*15.04/5+7.35
  Fuel_T =pulseIn(fuel_temp_pin, HIGH,32)/2040.0*5.0*20 + 273.15;
  // Assume: 273.15 + 20*V
  PCB_temp = pulseIn(pcb_temp_pin, HIGH, 32);//*250.0/204.0/1024.0
  ...
  */
   ...
   for (int i=0;i<7;i++){
    analogRead(0);
    N= 2040.0*5.0*26.155*-1 + 145.29 + 273.15;
   }
}
...
void loop() {
  int initial_time = micros(); //Start timer
  read_sensors(); //Reads all Sensors
  safety_checks(); //Checks PCB Temp and IPG/CPG 
  calc_injection(); 
  send_injection();
  int calc_time = micros() - initial_time; //End timer
  display_sensors();
  display_calc();
  Serial.print("Calculation time (microseconds): ");
